...
<head>
  ...
  <title>Modifier son offre</title>
...
<?php
    ...
    if(isset($_POST['submit'])){  
      ...
      $modifierOffre = mysqli_query($conn, $modifier);
      if(!$modifierOffre){
        ...
        $modifierCat = "UPDATE `categorie` SET `nom_cat` = '$categorie' WHERE `categorie`.`id_offre` = '$id';";
        $modifierCategorie = mysqli_query($conn, $modifierCat);
    ...
    $resultat = mysqli_query($conn, $sql);
    $row = mysqli_fetch_array($resultat);
    $selectionner_cat = "SELECT * FROM categorie WHERE id_offre ='$id';";
    $categorie = mysqli_query($conn, $selectionner_cat);
    $row2 = mysqli_fetch_array($categorie);
    $location = $row["image"];
        ...
        <div class="input-field">
          ...
          <div class="select">
            <select name="categorie" id="selectCat">
             <option value="<?php echo $row2["nom_cat"] ;?>">La categorie : <?php echo $row2["nom_cat"] ;?></option>
